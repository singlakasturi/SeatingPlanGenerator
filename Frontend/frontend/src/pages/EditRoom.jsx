...
//   const { roomData = MOCK_ALT_DATA, examData = { date: new Date().toISOString() }, onUpdate } =
//   location.state || {};
...
//     state: { roomData, examData, onUpdate },
//     replace: true
//   });
// };
...
//   const updatedRoom = {
//     ...roomData,
//     allocations: roomData.allocations.map(a => ({
//       ...a,
//       note: cellNotes[a.seatId] || ""
//     }))
//   };

//   // 🔥 update parent state (RoomAllocation)
//   if (onUpdate) {
//     onUpdate(examData.date, updatedRoom);
//   }
...
//   const handleNoteChange = (seatId, value) => {
//     setCellNotes(prev => ({
//       ...prev,
//       [seatId]: value
//     }));
...
//   const planDateText = examData?.date
//     ? new Date(examData.date).toLocaleString(undefined, { month: "long", year: "numeric" })
//     : "December 2025";
...
//     .desk-cell { background: white; padding: 4px; min-width: 70px; height: auto; display: flex; flex-direction: column; align-items: center; justify-content: center; font-size: 0.75rem; gap: 4px; }
    
//     @media print {
//       .preview-root { background: white; padding: 0; }
//       .header, .back-btn, .save-button-fixed, .cell-note-input { display: none; }
...
//                           <div className="cell-roll-display">{formatRoll(content)}</div>
//                           <input
//                             type="text"
//                             className="cell-note-input"
//                             placeholder="PWD/Note"
//                             value={cellNotes[seatId] || ""}
//                             onChange={(e) => handleNoteChange(seatId, e.target.value)}
//                             maxLength={15}
//                           />
//                         </div>
//                       ) : null}
...
//                       <tr key={rIdx}>
//                         {row.map((cell, cIdx) => (
//                           <td key={cIdx} className={cell.seat ? "seat-filled" : "seat-vacant"}>
//                             {cell.seat ? (
//                               <div className="cell-edit-wrapper">
//                                 <div className="cell-roll-display">{formatRoll(cell.seat.roll)}</div>
//                                 <input
//                                   type="text"
//                                   className="cell-note-input"
//                                   placeholder="PWD/Note"
//                                   value={cellNotes[cell.seatId] || ""}
//                                   onChange={(e) => handleNoteChange(cell.seatId, e.target.value)}
//                                   maxLength={15}
//                                 />
//                               </div>
//                             ) : (
...
//                 {seat ? (
//                   <div className="cell-edit-wrapper" style={{width: '100%'}}>
//                     <span className="font-bold text-slate-700">{seat.subjectShort}</span>
//                     <div className="cell-roll-display">{formatRoll(seat.rollNo)}</div>
//                     <input
//                       type="text"
//                       className="cell-note-input"
//                       placeholder="PWD/Note"
...
//         <button className="save-button-fixed" onClick={handleSave}>
//           Save Changes
//         </button>
//       </main>
//     </div>
    ...
    if (cached) {
      ...
      if (payload.results) {
        payload.results = payload.results.map(result => {
          if (result.date === examData.date) {
            return {
              ...result,
              response: {
                ...result.response,
                rooms: result.response.rooms.map(r =>
        ...
        sessionStorage.setItem("allocationPayload", JSON.stringify(payload));
      }
    }
  } catch (error) {
